...
void doMusic(const std::string& musicFilePath) {
    ...
    // Reproduce el sonido y no esperes su finalización en un loop infinito.
    tempEngine->play2D(musicFilePath.c_str(), false);
    // Aquí surge un problema: Si sales de la función de inmediato, el engine se destruye al salir 
    // (si es local) o si haces drop(), la reproducción se corta. Debes mantener el engine vivo 
